...
df_metrics = spark.read.format("delta").load("/lakehouse/dst/system/metrics/activity_log")

# Filter for job completion events
history_df = df_metrics.filter(col("metric_function") == "completion") \
...
volume_df = df_metrics.filter((col("metric_unit") == "rows") & (col("job_name") == "join_output_count")) \
